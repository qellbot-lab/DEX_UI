    ...
    <div className="page agents-page">
      <section className="catalog-hero chapter">
        ...
        <Reveal className="team-dock" delay={0.08}>
          <div className="team-dock-head"><span>当前战队</span><b>{team.length} / {TEAM_CAPACITY}</b></div>
          <div className="team-dock-slots">
            ...
            {Array.from({ length: TEAM_CAPACITY - team.length }, (_, index) => <EmptyAgentSlot key={index} index={team.length + index + 1} />)}
          </div>
      ...
      <section className="agent-inspection chapter">
        ...
        <Reveal className="inspection-metrics" delay={0.12}>
          ...
          <div className="inspection-actions"><Action onClick={() => demo.addAgent(selectedAgent.id)} variant="secondary" disabled={demo.selectedAgentIds.includes(selectedAgent.id) || demo.selectedAgentIds.length >= TEAM_CAPACITY}>加入战队</Action><Action onClick={() => navigate(`/agents/${selectedAgent.id}`)}>查看详情</Action></div>
        </Reveal>
      </section>
    </div>
  )
